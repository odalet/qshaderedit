...
QSize QShaderEdit::sizeHint() const { return QSize(600, 400); }

void QShaderEdit::about()
{
	// @@ Write a better about dialog.
	QMessageBox::about(this,
		tr("About QShaderEdit"),
...
void QShaderEdit::openRecentFile()
{
	...
	if (!m_document->close()) return;

	auto fileName = action->data().toString();
	if (!m_document->loadFile(fileName)) QMessageBox::critical(this,
		tr("Error"),
		tr("Cannot load file:\n") + fileName,
...
void QShaderEdit::onCursorPositionChanged()
{
	...
	m_statusLabel->setText(tr(" Line: %1  Col: %2").arg(line).arg(column));
}
...
void QShaderEdit::onTitleChanged(QString title) { updateWindowTitle(title); }

void QShaderEdit::onFileNameChanged(QString fileName) { addRecentFile(fileName); }

void QShaderEdit::onEffectCreated()
{
	auto effect = m_document->effect();
...
void QShaderEdit::onEffectBuilt(bool succeeded)
{
	auto effect = m_document->effect();
	Q_ASSERT(effect != NULL);
	...
	if (succeeded)
	{
		statusBar()->showMessage(tr("Compilation succeeded."), 2000);
...
void QShaderEdit::onParameterChanged() { m_scenePanel->refresh(); }
...
void QShaderEdit::createMenus()
{
	...
	for each (auto& kvp in services::themeManager->themes())
	{
		auto key = kvp.first;
		auto& theme = kvp.second;

		auto themeAction = themeMenu->addAction(theme.name);
		themeAction->setCheckable(true);
		connect(themeAction, &QAction::changed, [=]()
		{
			if (themeAction->isChecked())
				services::themeManager->switchTo(key);
		});

		themesGroup->addAction(themeAction);
...
void QShaderEdit::updateCheckedTheme()
{
	auto found = themeActions.at(services::themeManager->currentKey());
	if (found) found->setChecked(true);
}
