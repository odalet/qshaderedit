...
public:
	...
	{
		if (m_dlist != 0) glDeleteLists(m_dlist, 1);
	}
...
{
public:
	virtual QString name() const { return tr("Cube"); }
	virtual QString description() const { return tr("Cube"); }
	virtual QIcon icon() const { return QIcon(); }
	virtual Scene * createScene() const { return new CubeScene(); }
};
...
public:
	...
	~ObjScene()
	{
		if (m_dlistBase) glDeleteLists(m_dlistBase, m_dlistCount);
	}
	...
	virtual void setupMenu(QMenu * menu) const { Q_UNUSED(menu); }
...
private:
	...
	{
	public:
		~MaterialLib() { qDeleteAll(m_materials); }
	...
	void load(const QString & fileName)
	{
		...
		vec3 vmin(1e10, 1e10, 1e10);
		vec3 vmax(-1e10, -1e10, -1e10);

		auto defaultMaterial = new Material;
		MaterialLib* currentMaterialLib = nullptr;
		auto currentSurface = new Surface;
		...
		while (!file.atEnd())
		{
			...
			{
				auto faces = line.split(' ');
				QVector<Vertex> face;

				for (int i = 1; i < faces.size(); i++)
				{
					auto indices = faces[i].split('/');
					Vertex v;
			...
			{
				auto name = usemtlPattern.cap(1);
				Material* material = nullptr;

				if (currentMaterialLib) material = currentMaterialLib->material(name);
				if (!material)
					currentSurface = surfaces[0]; // default material
				else
				{
					currentSurface = nullptr;
					foreach(auto surf, surfaces)
					{
						Q_ASSERT(surf);
						Q_ASSERT(surf->material);
			...
			{
				auto name = mtllibPattern.cap(1);
				currentMaterialLib = nullptr;
				...
				if (!currentMaterialLib)
				{
					...
					auto mtlLibFileName = info.dir().filePath(name);
					auto mtlLib = new MaterialLib;
		...
		if (surfaces[0]->faces.isEmpty()) surfaces.remove(0);
		...
		for (GLuint n = 0; n < m_dlistCount; n++)
		{
			...
			auto surf = surfaces[n];
			surf->material->bind();

			for (int face = 0; face < surf->faces.count(); face++) 
			{
				auto verts = surf->faces[face];
				glBegin(GL_POLYGON);

				for (int vert = 0; vert < verts.count(); vert++) 
				{
					if (verts[vert].texcoord >= 0) glTexCoord2fv((GLfloat*)&texcoords[verts[vert].texcoord]);
					if (verts[vert].normal >= 0) glNormal3fv((GLfloat*)&normals[verts[vert].normal]);
...
{
public:
	virtual QString name() const { return tr("OBJ file"); }
	virtual QString description() const { return tr("WaveFront 3D Object"); }
	virtual QIcon icon() const { return QIcon(); }
	virtual Scene* createScene() const { return new ObjScene(); }
};
