...
namespace
{
	static QList<const EffectFactory*>* s_factoryList = nullptr;
}
...
{
	for each (auto factory in *s_factoryList)
	{
...
const QList<const EffectFactory *> & EffectFactory::factoryList()
{
	...
	if (s_factoryList == nullptr) return empty;
...
void EffectFactory::removeFactory(const EffectFactory* factory)
{
	...
	if (s_factoryList->isEmpty())
	{
		...
		s_factoryList = nullptr;
	}
}

void Effect::makeCurrent() { m_widget->makeCurrent(); }
void Effect::doneCurrent() { m_widget->makeCurrent(); }
